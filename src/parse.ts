	...
	nodes
		...
		.map((line) => {
			if (fieldRegex.test(line)) {
				...
				const modifiedLine = line.replace(childRegex, "$1"); // Remove the leading indentation
				children.push(modifiedLine);
			}
		});
...
export function parseNode(node: string) {
	...
	for (const obj of otherFields) {
		for (const key in obj) {
			if (Object.prototype.hasOwnProperty.call(joinedFields, key)) {
